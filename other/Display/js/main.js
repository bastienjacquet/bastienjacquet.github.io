...
function drawDescendant(indivData, inR_orig, startA_orig, endA_orig, generation, orient) {
	...
	for (var m = 0; m < nbSpouse; ++m) {
		...
		var inR = inR_orig;
		var outR_s = inR + (generation > param.expandStartDesc ? param.radiusRadial : param.radius)*param.spouseRadiusRatio;
		...
		for (var c = 0; c < indivData.marriages[m].children.length; ++c) {
			...
			drawPersCell(indivData.marriages[m].children[c], inR + param.padding / 2, outR - param.padding / 2, startA, endA, generation + 1, (generation + 1 > param.expandStartDesc ? 1 : orient), (generation + 1 > param.expandStartDesc ? 1 : 0), (startA > Math.PI ? 0 : Math.pow(2, generation + 2) / 2.0), false)
